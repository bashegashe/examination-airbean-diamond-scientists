...
function getPromotionCart(stateCart) {
    ...
    allPromotions.map((promotion) => {
        if (!Array.isArray(promotion.productsRequired)) promotion.productsRequired = [promotion.productsRequired];
    });
    ...
    allPromotions = allPromotions.filter((promotion) => {
        return promotion.productsRequired.every((currentPromotion) => {
            ...
            return promotionCart.some((cartItem) => {
                return cartItem.title === currentPromotion.name && cartItem.quantity >= currentPromotion.min
            });
        });
    });
    ...
    allPromotions.map((promotion, i) => {
        ...
        if (promotion.canBeCombinedWithOtherPromotions === false) noCombinationPromotionExists = true;
    });
    ...
    if (allPromotions.length > 1) {
        ...
        allPromotions.map((promotion) => {
            const combinationStrategy = promotion.combinationStrategy || 'highest price';

            promotion.productsRequired.map((product) => {
                if (product.name in promotionPrices) {
                    if (combinationStrategy === 'lowest price' && product.price < promotionPrices[product.name]) {
                        promotionPrices[product.name] = product.price;
                    } else if (combinationStrategy !== 'lowest price' && product.price > promotionPrices[product.name]) {
                        promotionPrices[product.name] = product.price;
                    }
                } else {
                    promotionPrices[product.name] = product.price;
    ...
    promotionCart.sort((a, b) => a.title !== b.title ? a.title < b.title ? -1 : 1 : 0);
...
function applyPromotion(promotion, promotionCart) {
    ...
    if (promotion.combineProducts === true) {
        ...
        const combinedProduct = promotion.productsRequired.reduce((acc, cur) => {
        ...
        let lowestOriginalQuantity = Math.min(...originalItems.map((cartItem) => cartItem.quantity));
        ...
        promotionCart.map((cartItem) => {
            if (promotion.productsRequired.some((product) => {
